...
export default function ExploreProducts() {
  const [products, setProducts] = useState<Product[]>([]); // renamed state variable
  const [displayedProducts, setDisplayedProducts] = useState<Product[]>([]);
  const [wishlist, setWishlist] = useState<Product[]>([]);
  const [showMoreVisible, setShowMoreVisible] = useState(true);
  ...
  useEffect(() => {
    async function fetchProducts() {
      const products: Product[] = await client.fetch(allProductsQuery); // using the query correctly
      setProducts(products); // correctly setting the products
      setDisplayedProducts(products.slice(0, 24)); // display first 24 products
      setShowMoreVisible(products.length > 24); // Show More button logic
    }

    fetchProducts();
    setWishlist(getWishlistItems()); // Fetch wishlist items
  ...
  const loadMoreProducts = () => {
    const currentLength = displayedProducts.length;
    const nextProducts = products.slice(currentLength, currentLength + 8); // load more products
    setDisplayedProducts([...displayedProducts, ...nextProducts]);

    if (displayedProducts.length + nextProducts.length >= products.length) {
      setShowMoreVisible(false); // hide show more if all products are displayed
    }
  };
